...
model_quad =Regression_Model(pipeline=(PolynomialFeatures(degree=2), LinearRegression()), model_name="Linear")
model_quad.fit(data_dict['train_features'], data_dict['train_outputs'])
y_pred = model_quad.model.predict(data_dict['test_features'])
...
models = [
    Regression_Model(pipeline=(PolynomialFeatures(degree=2), LinearRegression()), model_name="Linear"),
